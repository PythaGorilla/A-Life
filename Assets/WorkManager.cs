...
namespace Assets
{
    public static class WorkManager
    {
        ...
        public static List<WorldObject.WorldObject> FindNearbyObjects(Vector3 position, float range)
        {
            Collider[] hitColliders = Physics.OverlapSphere(position, range);
            HashSet<int> nearbyObjectIds = new HashSet<int>();
            List<WorldObject.WorldObject> nearbyObjects = new List<WorldObject.WorldObject>();
            for (int i = 0; i < hitColliders.Length; i++)
            {
                Transform parent = hitColliders[i].transform.parent;
                if (parent)
                {
                    WorldObject.WorldObject parentObject = parent.GetComponent<WorldObject.WorldObject>();
    ...
    public static WorldObject.WorldObject FindNearestWorldObjectInListToPosition(List<WorldObject.WorldObject> objects, Vector3 position)
    {
